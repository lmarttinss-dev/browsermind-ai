...
export const api = {
  ...
  analyze: (params: { prompt: string; model: string; pageContent?: string; screenshot?: string; templateId?: string; qnaContent?: string }) =>
  ...
  updateSupplierReport: (productId: string, supplierIndex: number, report: string, supplierUrl?: string) =>
  ...
  analyzeMarket: (id: string, params: { email?: string; model?: string; prompt: string }) =>
    request<{ success: boolean; report: string; product: PipelineProduct }>(`/api/pipeline/${id}/analyze-market`, {
      method: "POST",
