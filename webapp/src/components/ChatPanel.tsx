        ...
        {activeTemplate === MARKET_TEMPLATE_ID && (
          <div className="relative">
            <select
              ...
              className="w-full appearance-none bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 pr-8 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer truncate"
            >
              <option value="">Vincular a um produto da esteira (opcional)...</option>
              {pipelineProducts.map((p) => {
                const cleanTitle = p.title.replace(/\*+/g, "")
                const truncated = cleanTitle.length > 60 ? cleanTitle.slice(0, 60) + "…" : cleanTitle
                return (
                  <option key={p._id} value={p._id} title={cleanTitle}>
                    {truncated} ({p.stage})
                  </option>
                )
              })}
