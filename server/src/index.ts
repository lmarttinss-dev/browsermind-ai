...
// Solicita à IA uma seção obrigatória que foi omitida no relatório, reutilizando
// o mesmo modelo. Retorna o texto gerado ou null em caso de falha/indisponibilidade.
async function requestMissingSection(
  model: string,
  systemPrompt: string,
  userMessage: string,
  aiResponse: string,
  ...
  // Mantém apenas o final do relatório no contexto (limite de tokens)
  const previousReport = aiResponse.slice(-60000)
  ...
  try {
        ...
        `https://generativelanguage.googleapis.com/v1beta/models/${geminiModel}:generateContent?key=${key}`,
        {
          contents: [{
            parts: [
              { text: systemPrompt },
              { text: userMessage },
              { text: `Relatório gerado anteriormente:\n${previousReport}` },
              { text: missingPrompt },
            ],
          }],
      ...
      const r = await axios.post(
        "https://api.openai.com/v1/chat/completions",
        {
          ...
          messages: [
            ...
            { role: "assistant", content: previousReport },
    ...
    if (model === "deepseek-flash" || model === "deepseek-pro") {
    ...
    console.log("⚠️ Falha ao solicitar seção faltante:", err instanceof Error ? err.message : err)
    return null
  }
}
...
app.post("/api/analyze", async (req, res) => {
  try {
    ...
    // Garante a presença da seção "Demanda Recente" no relatório de anúncio
    // (a IA às vezes a omite ao pular direto para o bloco catálogo/independente).
    if (templateId === "analise-anuncio-independente" && !/demanda\s+recente/i.test(aiResponse)) {
      const missing = await requestMissingSection(
        model,
        SYSTEM_PROMPT,
        userMessage,
        aiResponse,
        `Gere AGORA, em Markdown, a seção "## 📈 Demanda Recente (Velocidade de Vendas)" que ficou faltando no relatório. Siga EXATAMENTE o template desta seção (Vendas por dia, Vendas mensais, Total vendido, Data de criação, Idade do anúncio, Média histórica por dia, Comparação, Estoque atual, classificação 🔥 Forte/✅ Ativa/⚠️ Fraca/❌ Estagnada, Avaliações recentes e Veredito). Retorne apenas a seção, começando pelo título "## 📈 Demanda Recente (Velocidade de Vendas)".`
      )
      if (missing) {
        const branchIndex = aiResponse.search(/^#{1,3}\s*SE FOR\s*[📚📦]/im)
        if (branchIndex !== -1) {
          aiResponse = aiResponse.slice(0, branchIndex) + missing.trim() + "\n\n" + aiResponse.slice(branchIndex)
        } else {
          aiResponse += "\n\n" + missing.trim()
        }
      }
...
Classifique o fornecedor exclusivamente em UMA das duas categorias:
...
2. Considere o conjunto das evidências: Business Type, Company Type, informações de fabricação, produtos oferecidos, variedade de categorias, fotos/vídeos da fábrica, endereço e instalações, número de funcionários, capacidade produtiva, OEM/ODM, licença comercial e descrição da empresa.
...
6. NÃO crie uma terceira categoria.
